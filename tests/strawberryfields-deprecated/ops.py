...
def single_mode_gate(matrix, mode, in_modes, pure=True, batched=False):
    ...
    if not pure:
        ...
        einsum_inputs.append(tf.transpose(tf.compat.v1.conj(matrix), transposed_axis))
    print('debug', eqn)
...
def kerr_interaction(kappa, mode, in_modes, D, pure=True, batched=False):
    """returns Kerr unitary matrix on specified input modes (in_modes | Circuit._state)"""
